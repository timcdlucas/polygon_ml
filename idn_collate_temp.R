...
files2 <- files2[file.exists(files2)]
cv1_output2_list <- lapply(files2, function(x) get(load(x)))
cv1_output2 <- collate(cv1_output2_list)

data_cv1_idn_sub <- data_cv1_idn[-5]
class(data_cv1_idn_sub) <- c("ppj_cv", "list")

obspred_map(data_cv1_idn_sub, cv1_output2, column = TRUE)
ggsave('figs/idn_polygons_only_obspred_map.png')
obspred_map(data_cv1_idn_sub, cv1_output2, trans = 'log10', column = TRUE)
...
files3 <- files3[file.exists(files3)]

cv1_output3_list <- lapply(files3, function(x) get(load(x)))
cv1_output3 <- collate(cv1_output3_list)

data_cv1_idn_sub2 <- data_cv1_idn[-4]
class(data_cv1_idn_sub2) <- c("ppj_cv", "list")
...
files6 <- files6[file.exists(files6)]

cv2_output3_list <- lapply(files6, function(x) get(load(x)))
cv2_output3 <- collate(cv2_output3_list)
data_cv2_idn_sub <- data_cv2_idn[-3]
class(data_cv2_idn_sub) <- c("ppj_cv", "list")


obspred_map(data_cv2_idn_sub, cv2_output3, column = FALSE)
ggsave('figs/idn_joint_obspred_map2.png')
obspred_map(data_cv2_idn_sub, cv2_output3, trans = 'log10', column = FALSE)
