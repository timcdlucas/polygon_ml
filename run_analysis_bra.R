...
ml_local_raster_paths <- c(
  'model_outputs/ml_pred_rasters/madagascar_bra_enet.tif',
  'model_outputs/ml_pred_rasters/madagascar_bra_gbm.tif',
  'model_outputs/ml_pred_rasters/madagascar_bra_nnet.tif',
  'model_outputs/ml_pred_rasters/madagascar_bra_ppr.tif',
  'model_outputs/ml_pred_rasters/madagascar_bra_ranger.tif'
)


ml_global_raster_paths <- c(
  'model_outputs/ml_pred_rasters/global_bra_enet.tif',
  'model_outputs/ml_pred_rasters/global_bra_xgbTree.tif',
  'model_outputs/ml_pred_rasters/global_bra_nnet.tif',
  'model_outputs/ml_pred_rasters/global_bra_ppr.tif',
  'model_outputs/ml_pred_rasters/global_bra_ranger.tif'
)
...
data_bra_ml <- process_data(
  ...
  useiso3 = 'BRA',
  transform = NULL)
save(data_bra_ml, file = 'model_outputs/bra_ml_data.RData')


data_bra_mlg <- process_data(
  ...
  useiso3 = 'BRA',
  transform = NULL)
save(data_bra_mlg, file = 'model_outputs/bra_mlg_data.RData')

data_bra_all <- process_data(
...
save(data_bra_all, file = 'model_outputs/bra_all_data.RData')


autoplot(data_bra_cov, pr_limits = c(0, 0.3))
autoplot(data_bra_cov, pr_limits = c(0, 0.3), trans = 'log1p')

ggsave('figs/bra_input_data.png')
...
data_cv1_bra <- cv_random_folds(data_bra_cov, k = 6)
data_cv1_bra_ml <- cv_random_folds(data_bra_ml, k = 6, 
                                   polygon_folds = attr(data_cv1_bra, 'polygon_folds'),
                                   pr_folds = attr(data_cv1_bra, 'pr_folds'))
data_cv1_bra_mlg <- cv_random_folds(data_bra_ml, k = 6, 
                                    polygon_folds = attr(data_cv1_bra, 'polygon_folds'),
                                    pr_folds = attr(data_cv1_bra, 'pr_folds'))
data_cv1_bra_all <- cv_random_folds(data_bra_all, k = 6, 
                                    polygon_folds = attr(data_cv1_bra, 'polygon_folds'),
                                    pr_folds = attr(data_cv1_bra, 'pr_folds'))


autoplot(data_cv1_bra, jitter = 0)
autoplot(data_cv1_bra_ml, jitter = 0)

ggsave('figs/bra_cv_random.png')
save(data_cv1_bra, file = 'model_outputs/bra_cv_1.RData')


# Spatial
data_cv2_bra <- cv_spatial_folds(data_bra_cov, k = 3)
data_cv2_bra_ml <- cv_spatial_folds(data_bra_ml, k = 3, 
                                    polygon_folds = attr(data_cv2_bra, 'polygon_folds'),
                                    pr_folds = attr(data_cv2_bra, 'pr_folds'))
data_cv2_bra_mlg <- cv_spatial_folds(data_bra_mlg, k = 3, 
                                     polygon_folds = attr(data_cv2_bra, 'polygon_folds'),
                                     pr_folds = attr(data_cv2_bra, 'pr_folds'))
data_cv2_bra_all <- cv_spatial_folds(data_bra_all, k = 3, 
                                     polygon_folds = attr(data_cv2_bra, 'polygon_folds'),
                                     pr_folds = attr(data_cv2_bra, 'pr_folds'))

autoplot(data_cv2_bra, jitter = 0)
autoplot(data_cv2_bra_ml, jitter = 0)

ggsave('figs/bra_cv_spatial2.png')
save(data_cv2_bra, file = 'model_outputs/bra_cv_2.RData')


#autoplot(data_cv1_bra[[1]]$train, pr_limits = c(0, 0.3))
...
if(FALSE){
  ...
  full_model_ml <- fit_model(data_bra_ml, mesh_bra, its = 1000, model.args = arg_list)
  ...
  ggsave('figs/bra_full_model_ml_in_sample.png')
  
  save(full_model_ml, file = 'model_outputs/full_model_ml_bra.RData')
  
  
  
  
  
  full_model_all <- fit_model(data_bra_all, mesh_bra, its = 1000, model.args = arg_list)
  ...
  in_sample_all <- cv_performance(predictions = full_model_all$predictions, 
                                  holdout = data_bra_all,
                                  model_params = full_model_all$model, 
                                  CI = 0.8,
                                  use_points = use_points)
  autoplot(in_sample_all, CI = TRUE)
  autoplot(in_sample_all, trans = 'log1p', CI = TRUE)
  ggsave('figs/bra_full_model_all_in_sample.png')
  
  save(in_sample_all, file = 'model_outputs/full_model_all_bra_all.RData')
...
cv1_output1 <- run_cv(data_cv1_bra, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 20, cores = 6)
obspred_map(data_cv1_bra, cv1_output1, column = FALSE, mask = TRUE)
ggsave('figs/bra_covs_only_obspred_map.png')
obspred_map(data_cv1_bra, cv1_output1, trans = 'log10', column = FALSE, mask = TRUE)
ggsave('figs/bra_covs_oonly_obspred_map_log.png')
autoplot(cv1_output1, type = 'obs_preds', CI = F)
ggsave('figs/bra_covs_oonly_obspred.png')
autoplot(cv1_output1, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_covs_only_obspred_log.png')
save(cv1_output1, file = 'model_outputs/bra_covs_cv_1.RData')

cat('Start cv1 model 2\n')

cv1_output2 <- run_cv(data_cv1_bra_ml, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 40, cores = 6)
obspred_map(data_cv1_bra, cv1_output2, column = FALSE)
ggsave('figs/bra_ml_only_obspred_map.png')
obspred_map(data_cv1_bra, cv1_output2, trans = 'log10', column = FALSE)
ggsave('figs/bra_ml_only_obspred_map_log.png')
autoplot(cv1_output2, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_ml_only_obspred.png')
autoplot(cv1_output2, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_ml_only_obspred_log.png')
save(cv1_output2, file = 'model_outputs/bra_ml_cv_1.RData')

cat('Start cv1 model 3\n')

cv1_output3 <- run_cv(data_cv1_bra_all, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 20, cores = 6)
obspred_map(data_cv1_bra, cv1_output3, column = FALSE)
ggsave('figs/bra_all_obspred_map.png')
obspred_map(data_cv1_bra, cv1_output3, trans = 'log10', column = FALSE)
ggsave('figs/bra_all_obspred_map_log.png')
autoplot(cv1_output3, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_all_obspred.png')
autoplot(cv1_output3, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_all_obspred_log.png')

save(cv1_output3, file = 'model_outputs/bra_all_cv_1.RData')


cat('Start cv1 model 4\n')

cv1_output4 <- run_cv(data_cv1_bra_mlg, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 20, cores = 6)
obspred_map(data_cv1_bra, cv1_output4, column = FALSE)
ggsave('figs/bra_mlg_obspred_map.png')
obspred_map(data_cv1_bra, cv1_output4, trans = 'log10', column = FALSE)
ggsave('figs/bra_mlg_obspred_map_log.png')
autoplot(cv1_output4, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_mlg_obspred.png')
autoplot(cv1_output4, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_mlg_obspred_log.png')

save(cv1_output3, file = 'model_outputs/bra_mlg_cv_1.RData')
...
cv2_output1 <- run_cv(data_cv2_bra, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 40, cores = 3)
obspred_map(data_cv2_bra, cv2_output1, column = FALSE, mask = TRUE)
ggsave('figs/bra_covs_only_obspred_map2.png')
obspred_map(data_cv2_bra, cv2_output1, trans = 'log10', column = FALSE, mask = TRUE)
ggsave('figs/bra_covs_only_obspred_map_log2.png')
autoplot(cv2_output1, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_covs_only_obspred2.png')
autoplot(cv2_output1, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_covs_only_obspred_log2.png')

save(cv2_output1, file = 'model_outputs/bra_covs_cv_2.RData')

cat('Start cv2 model 2')

cv2_output2 <- run_cv(data_cv2_bra_ml, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 30, cores = 3)
obspred_map(data_cv2_bra, cv2_output2, column = FALSE, mask = TRUE)
ggsave('figs/bra_ml_only_obspred_map2.png')
obspred_map(data_cv2_bra, cv2_output2, trans = 'log10', column = FALSE, mask = TRUE)
ggsave('figs/bra_ml_only_obspred_map_log2.png')
autoplot(cv2_output2, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_ml_only_obspred2.png')
autoplot(cv2_output2, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_ml_only_obspred_log2.png')

save(cv2_output2, file = 'model_outputs/bra_ml_cv_2.RData')

cat('Start cv2 model 3')

cv2_output3 <- run_cv(data_cv2_bra_all, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 0, cores = 3)
obspred_map(data_cv2_bra, cv2_output3, column = FALSE, mask = TRUE)
ggsave('figs/bra_all_obspred_map2.png')
obspred_map(data_cv2_bra, cv2_output3, trans = 'log10', column = FALSE, mask = TRUE)
ggsave('figs/bra_all_obspred_map_log2.png')
autoplot(cv2_output3, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_all_obspred2.png')
autoplot(cv2_output3, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_all_only_obspred_log2.png')

save(cv2_output3, file = 'model_outputs/bra_all_cv_2.RData')
...
cv2_output4 <- run_cv(data_cv2_bra_mlg, mesh_bra, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 0, cores = 3)
obspred_map(data_cv2_bra, cv2_output4, column = FALSE, mask = TRUE)
ggsave('figs/bra_mlg_obspred_map2.png')
obspred_map(data_cv2_bra, cv2_output4, trans = 'log10', column = FALSE, mask = TRUE)
ggsave('figs/bra_mlg_obspred_map_log2.png')
autoplot(cv2_output4, type = 'obs_preds', CI = FALSE)
ggsave('figs/bra_mlg_obspred2.png')
autoplot(cv2_output4, type = 'obs_preds', CI = FALSE, tran = 'log1p')
ggsave('figs/bra_mlg_only_obspred_log2.png')

save(cv2_output3, file = 'model_outputs/bra_mlg_cv_2.RData')
