...
data_mlcov_cov <- load_data(PR_path, 
                          API_path, 
                          pop_path, 
...
data_sen_covml <- process_data(
  binomial_positive = data_mlcov_cov$pr$positive,
  binomial_n = data_mlcov_cov$pr$examined,
  coords = data_mlcov_cov$pr[, c('longitude', 'latitude')],
  polygon_response = data_mlcov_cov$api$api_mean,
  polygon_population = data_mlcov_cov$api$population,
  shapefile_id = data_mlcov_cov$api$shapefile_id,
  shps_id_column = 'area_id',
  shapefiles = data_mlcov_cov$shapefiles,
  pop_raster = data_mlcov_cov$pop,
  cov_rasters = data_mlcov_cov$covs,
  useiso3 = 'SEN',
  transform = c(4:7),
  scale_rasters = TRUE)

save(data_sen_covml, file = 'model_outputs/sen_covml_data.RData')
...
data_cv1_sen_all <- cv_random_folds(data_sen_all, k = 6, 
                                    polygon_folds = attr(data_cv1_sen, 'polygon_folds'),
                                    pr_folds = attr(data_cv1_sen, 'pr_folds'))
data_cv1_sen_covml <- cv_random_folds(data_sen_covml, k = 6, 
                                    polygon_folds = attr(data_cv1_sen, 'polygon_folds'),
                                    pr_folds = attr(data_cv1_sen, 'pr_folds'))
...
data_cv2_sen_all <- cv_spatial_folds(data_sen_all, k = 5, 
                                     polygon_folds = attr(data_cv2_sen, 'polygon_folds'),
                                     pr_folds = attr(data_cv2_sen, 'pr_folds'))
data_cv2_sen_covml <- cv_spatial_folds(data_sen_covml, k = 5, 
                                     polygon_folds = attr(data_cv2_sen, 'polygon_folds'),
                                     pr_folds = attr(data_cv2_sen, 'pr_folds'))
...
cv1_output5 <- run_cv(data_cv1_sen_covml, mesh_sen, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 20, cores = 6)

save(cv1_output5, file = 'model_outputs/sen_covml_cv_1.RData')

cv2_output5 <- run_cv(data_cv2_sen_covml, mesh_sen, its = 1000, 
                      model.args = arg_list, CI = 0.8, parallel_delay = 0, cores = 5)
save(cv2_output5, file = 'model_outputs/sen_covml_cv_2.RData')
