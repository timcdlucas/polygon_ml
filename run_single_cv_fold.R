...
if(cv_type == 'random'){
  ...
  } else if(data_type == 'ml'){
    cv_data <- get(load('model_outputs/idn_cv_1_ml.RData'))
  } else if(data_type == 'all'){
    cv_data <- get(load('model_outputs/idn_cv_1_all.RData'))
  } else { 
  ...
  if(data_type == 'covs'){
    ...
    cv_data <- get(load('model_outputs/idn_cv_2_ml.RData'))
  } else if(data_type == 'all'){
    cv_data <- get(load('model_outputs/idn_cv_2_all.RData'))
  } else { 
    stop('Unknown data type')
  }
} else {
  stop('Unknown cv type')
}
...
save(out, file = paste0('model_outputs/idn-', cv_type, '-', data_type, '-', i, '.RData'))
